...
class AnagramTester(object):
    ...
    def check_output(self, actual_output, expected_output):
        for line_number in range(len(actual_output)):
            ...
            if actual_set.intersection(expected_set) != expected_set:
                self.logger.debug(f"Mismatch: expected_line={expected_list} actual_line={actual_list}")
                return False
        return True
    ...
    def test_app(self):
        for test_file in TEST_FILES:
            ...
            runner.run()
            self.check_output(actual_output=runner.get_output(), expected_output=self.get_test_data_file(test_file))
